...
pub fn note_mapper(note: &str) -> Result<String, Box<dyn Error>> {
    dotenv().ok();
        ...
        if line.starts_with("plz") {
            ...
            if mapper.len() == 6usize {
                mapper_de.push(MapperDE {
                    ...
                    note: mapper[1].replace(" ", "-").to_string(),
    ...
    for i in mapper_de.iter() {
        if i.note.split("-").collect::<Vec<_>>()[0].to_string()
            == note.to_string().replace(" ", "-").to_string()
        {
